...
| `gueyBalance` | `double` | GUEY balance (on-chain, indexer, or ledger). |
...
- **Wallet:** `connectWallet` today only stores address locally. Backend should support **link wallet** (sign message) and return updated `walletAddress` + `gueyBalance`.
...
| `SettingsWallet` | 2, 3, 4 | `userName`, `gueyBalance`, `onConnectWallet` stub, `signOut` |
...
| `SettingsWallet` | 3–5 | `userName`, `companyName`, `gueyBalance`, wallet stub, `signOut` |
...
| `MobileSettings` | 3 | `userName`, `companyName`, `gueyBalance`, `walletAddress`, connect wallet stub, `signOut` |
...
| `DroneEarningsScreen` | 2 | `gueyBalance` from state |
