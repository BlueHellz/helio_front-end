# KOOYOH — API contract (derived from Flutter client)
...
| `kyhBalance` | `double` | **KOO-YOH Coin (KYH)** balance on Solana (indexer or ledger). |
...
- **Wallet:** `connectWallet` today only stores address locally. Backend should support **link wallet** (sign message) and return updated `walletAddress` + `kyhBalance`.
- **Persistence:** Persist Solana-linked wallets and KYH balances in table **`kooyoh_wallets`** (do not use **`guey_wallets`** for KYH). **GUEY Coin** (native L1) uses separate storage when that chain is live.
...
| `SettingsWallet` | 2, 3, 4 | `userName`, `kyhBalance`, `onConnectWallet` stub, `signOut` |
...
| `SettingsWallet` | 3–5 | `userName`, `companyName`, `kyhBalance`, wallet stub, `signOut` |
...
| `MobileSettings` | 3 | `userName`, `companyName`, `kyhBalance`, `walletAddress`, connect wallet stub, `signOut` |
...
| `DroneEarningsScreen` | 2 | `kyhBalance` from state |
